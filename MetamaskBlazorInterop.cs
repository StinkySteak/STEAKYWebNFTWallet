...
namespace Nethereum.Metamask.Blazor
{
    public class MetamaskBlazorInterop : IMetamaskInterop
    {
        ...
        {
            var result = await _jsRuntime.InvokeAsync<int>("NethereumMetamaskInterop.GetSelectedNetwork");
            return result;
        }
        ...
        [JSInvokable()]
        public static async Task SelectedNetworkChanged(int selectedChain)
        {
            await MetamaskHostProvider.Current.ChangeSelectedNetworkAsync(selectedChain);
        }
    }
}