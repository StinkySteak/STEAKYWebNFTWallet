    ...
    EnableEthereum: async () => {
        try {
            ...
            ethereum.on("accountsChanged",
                function (accounts) {
                    DotNet.invokeMethodAsync('STEAKYWebNFTWallet', 'SelectedAccountChanged', accounts[0]);
                    console.log("accountsChanged triggered");
                });
            ethereum.on("chainChanged",
                function (chainId) {
                    DotNet.invokeMethodAsync('STEAKYWebNFTWallet', 'SelectedNetworkChanged', parseInt(chainId, 16));
                    console.log("chainChainged triggered");
                });
            return accounts[0];
        } catch (error) {
            return null;
